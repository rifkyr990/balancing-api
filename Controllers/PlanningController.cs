...
public class PlanningController : ControllerBase
{
    ...
    public async Task<IActionResult> CreatePlanning([FromBody] CreatePlanningRequest req)
    {
        ...
        for (int i = 0; i < req.Slots.Count; i++)
        {
            ...
            {
                return BadRequest(new { message = $"Jumlah slot tidak boleh negatif" });
            }
        }
        ...
        var newPlanning = new Planning
        {
            ...
            CandidateToken = string.IsNullOrEmpty(req.CandidateToken) ? "VEH-RIFKY" : req.CandidateToken,
    ...
    {
        var listData = await _context.Plannings.Include(p => p.Slots).OrderByDescending(p => p.CreatedAt).ToListAsync();
    ...
    {
        var detail = await _context.Plannings.Include(p => p.Slots).FirstOrDefaultAsync(p => p.PlanningId == id);
